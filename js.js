(function() {
    ...
    function createUI(wrap, originalBtn) {
        ...
        // === НОВОЕ: Кнопка "!" ===
        const exclamationBtn = createBtn('ws-exclamation-btn', '!');
        exclamationBtn.title = 'Добавить ! перед каждым словом и выполнить поиск';
        exclamationBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            
            const input = document.querySelector('.textinput__control');
            if (!input) {
                showNotify('Поле ввода не найдено');
                return;
            }
            
            let text = input.value.trim();
            if (!text) {
                showNotify('Введите текст для поиска');
                input.focus();
                return;
            }
            
            // Добавляем ! перед каждым словом и оборачиваем в кавычки
            const words = text.split(/\s+/).filter(Boolean);
            const modified = '"' + words.map(w => '!' + w).join(' ') + '"';
            
            input.value = modified;
            
            // Кликаем по кнопке поиска
            const searchBtn = document.querySelector('button.wordstat__search-button');
            if (searchBtn) {
                searchBtn.click();
            } else {
                showNotify('Кнопка поиска не найдена');
            }
        });
        // === КОНЕЦ НОВОГО ===
        ...
        // ==========================================
        // ВСТАВКА КНОПОК В DOM
        // ==========================================
        // === НОВОЕ: вставляем кнопку "!" перед кнопкой Статистика ===
        wrap.insertBefore(exclamationBtn, statsBtn);
        // === КОНЕЦ НОВОГО ===
